...
EOF
    ...
    while true; do
        curl -s https://download.hyper.space/api/install | bash | tee ~/hyperspace_install.log

        if ! grep -q "Failed to parse version from release data." ~/hyperspace_install.log; then
    ...
    while true; do
        aios-cli models add hf:TheBloke/phi-2-GGUF:phi-2.Q4_K_M.gguf | tee ~/model_download.log

        if grep -q "Download complete" ~/model_download.log; then
    ...
    echo "✅ Private key saved to ~/my.pem"
